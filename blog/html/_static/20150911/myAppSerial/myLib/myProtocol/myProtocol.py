...
#attributi
		...
		self.prs = None         # riferimento del parser usato
...
#protocol system
	...
	def sendListCommands(self, data, sto="\r", log=1):
		""" send Commands
		"""
		for cmnd in data:
			if self.asc:
				# print "ascii"
				cmd = self.sta + cmnd + sto
			else:
				# print "binary"
				cmd = cmnd
			# send comand
	...
	def write(self, data):
		...
		if self.mySer.ser:
			...
			self.sendListCommands([data,], sto="\r", log=0)
...
if __name__ == "__main__":
	...
	if choi == 1:
		...
		self.sendListCommands(["cmd_1",], sto="\r")
